...
def display_model_behavior(fE=(0,1,0.01), k=(0.01,10,0.01), E_0=(0,10,1), R_0=(0,10,1), A_0=(0,10,1), duration=(2.5,80,2.5)):
    ...
    f, (ax1,ax2) = plt.subplots(ncols=2, nrows=1, figsize=(15,5))
    ax1.semilogy(t, E, 'g', label='E')
    ax1.semilogy(t, R, 'b', label='R')
    ax1.semilogy(t, A, 'm', label='A')
    ax1.semilogy(t, V, 'k', label='M or V')
    ax1.semilogy([t[0], t[-1]], np.exp(np.polyval(growth_fit, [t[0], t[-1]])), '--r')
    ax1.set_title(f'Growth rate at the end: {growth_fit[0]:.3f}')    
    ax1.legend()
    ax1.set_ylabel('Amount or mass')
    ax2.plot(t, e, 'g', label='e')
    ax2.plot(t, r, 'b', label='r')
    ax2.plot(t, a, 'm', label='a')
    ax2.plot(t, e + r + a, '--k')
    ax2.legend()
    ax2.set_ylabel('concentration or mass fraction')
    for ax in [ax1, ax2]:
        ax.set_xlabel('Time')
    plt.show()
